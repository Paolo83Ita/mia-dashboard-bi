...
# ==========================================================================
# 1. CONFIGURAZIONE & STILE (v41.3 - Fix Gemini 429 & Restore Detail)
# ==========================================================================
st.set_page_config(
...
# CSS ottimizzato per Card e Tabelle
st.markdown("""
<style>
    .block-container {
...
def get_drive_files_list():
    """Recupera lista file gestendo il ritorno del service."""
    try:
        service, error = get_google_service()
        if error: return None, f"Errore Connessione Drive: {error}"
        if not service: return None, "Servizio Drive non inizializzato."
...
def convert_df_to_excel(df):
    ...
    output = io.BytesIO()
    # Reset index se è MultiIndex per evitare problemi in export
...
def smart_analyze_and_clean(df_in, page_type="Sales"):
    ...
    for col in df.columns:
        ...
        # Gestione Colonne Protette (Testo/Codici)
...
# ==========================================================================
# 4. AI CHAT ENGINE (FIXED: Gemini 1.5 & Retry Logic)
# ==========================================================================
def get_ai_response(user_query):
    try:
        import google.generativeai as genai
    except ImportError:
        return "⚠️ Modulo AI non installato correttamente. Controlla requirements.txt."
    ...
    if "gemini_api_key" not in st.secrets:
        return "🔑 Configura API Key nei Secrets."

    genai.configure(api_key=st.secrets["gemini_api_key"])
    
    # FIX IMPORTANTE: Usiamo gemini-1.5-flash perché 2.0-flash ha limiti di quota
    ...
    for attempt in range(max_retries):
        try:
            ...
            return local_vars.get('result', "❌ Nessun risultato generato dal codice AI.")
            
        except Exception as e:
            error_msg = str(e)
            # Gestione errore Quota (429)
            if "429" in error_msg or "Quota exceeded" in error_msg or "Resource exhausted" in error_msg:
                if attempt < max_retries - 1:
                    ...
                    return f"⚠️ Limite richieste AI raggiunto (Quota 429). Riprova tra un minuto. Dettaglio: {error_msg}"
            else:
                return f"🚨 Errore AI: {error_msg}"
...
# --------------------------------------------------------------------------
    ...
    if df_purch is not None:
        ...
        # Restore detailed table view from snippet
        st.subheader("📋 Dettaglio Righe Acquisto")
        cols_to_show_dettaglio = [
            'Purchase order', cpu_date, cpu_supp,
            'Part description', 'Part group description',
            cpu_qty, 'Received quantity', cpu_amt, cpu_kg
        ]
        # Filter for existing columns
        cols_present_dettaglio = [c for c in cols_to_show_dettaglio if c in df_pu_glob.columns]

        st.dataframe(
            df_pu_glob[cols_present_dettaglio],
            column_config={
                cpu_date: st.column_config.DateColumn("Data Ordine"),
                cpu_amt: st.column_config.NumberColumn("Importo Fatt.", format="€ %.2f"),
                cpu_kg: st.column_config.NumberColumn("Kg Calc.", format="%.0f"),
                cpu_qty: st.column_config.NumberColumn("Qta Ord.", format="%.0f"),
            },
        ...
        st.download_button(
            ...
            data=convert_df_to_excel(df_pu_glob[cols_present_dettaglio]),
